...
//static String f = "../../data/testfiles/nci100.smiles";
    ...
    static void noMolCreation() throws IOException {
        ...
        try (Stream<MoleculeObject> stream = MoleculeObjectUtils.createStream(new FileInputStream(f), false)) {
    ...
    static void simpleParallel() throws IOException {
        ...
        try (Stream<MoleculeObject> stream = MoleculeObjectUtils.createStream(new FileInputStream(f), true)) {
            long t0 = System.currentTimeMillis();
            long results = stream
                    .map(mo -> MoleculeUtils.fetchMolecule(mo, false))
                    .count();
