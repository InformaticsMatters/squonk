...
public abstract class DefaultHandler<T> implements HttpHandler<T>, VariableHandler<T> {
    ...
    protected T create(String mediaType, Class genericType, String name, InputStream input) throws Exception {
        SquonkDataSource ds = new InputStreamDataSource(name, mediaType, input, null);
        return create(ds);
    }
