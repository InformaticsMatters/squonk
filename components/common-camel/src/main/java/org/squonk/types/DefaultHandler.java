...
public abstract class DefaultHandler<T> implements HttpHandler<T>, VariableHandler<T> {
     ...
     */
    public T create(InputStream input) throws Exception {
        Constructor<T> constructor;
        T value;
        // first look for constructor that specifies generic type and InputStream
        if (genericType != null) {
            value = Utils.instantiate(getType(), new Class[] {Class.class, InputStream.class}, new Object[] {genericType, input});
            if (value != null) {
                return value;
            }
        }
        // OK, we'll try a constructor with just an InputStream
        value = Utils.instantiate(getType(), new Class[] {InputStream.class}, new Object[] {input});
        if (value != null) {
            return value;
        }
     ...
     * @throws Exception
     */
    public T create(Map<String,InputStream> inputs) throws Exception {
        ...
        if (inputs.size() == 1) {
            InputStream input = inputs.values().iterator().next();
            return create(input);
        } else {
    ...
    public T createMultiple(Map<String,InputStream> inputs) throws Exception {
