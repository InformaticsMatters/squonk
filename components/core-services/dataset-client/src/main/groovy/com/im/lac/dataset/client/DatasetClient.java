...
//    }
     ...
     * private getContentsAsStream() to get an idea of what the problem is (the need to throw a
     ...
     */
    public <T> Stream<T> getContentsAsStream(DataItem item, Class<T> type) throws IOException {
        HttpGet httpGet = new HttpGet(base + "/" + item.getId() + "/content");
        final CloseableHttpResponse response = httpclient.execute(httpGet);
        LOG.fine(response.getStatusLine().toString());
        HttpEntity entity1 = response.getEntity();
        InputStream is = entity1.getContent();
        return jsonHandler.streamFromJson(is, type, item.getMetadata(), true);
    }

    private Stream getContentsAsStream(DataItem item) throws ClassNotFoundException, IOException {
        Class cls = Class.forName(item.getMetadata().getClassName());
        return getContentsAsStream(item, cls);
    }
