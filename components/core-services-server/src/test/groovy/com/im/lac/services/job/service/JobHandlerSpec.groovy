...
class JobHandlerSpec extends Specification {
    ...
    void "test get statuses"() {
        ...
        Date now = new Date()
        store.putJob(new TestJob(new JobStatus("jobId1", "nobody", JobStatus.Status.COMPLETED, 0, 0, 0, now, now, null, null, null)))
    ...
    void "test purge old"() {
        ...
        store.putJob(new TestJob(new JobStatus("jobId1", "nobody", JobStatus.Status.COMPLETED, 0, 0, 0, now, now, null, null, null)))
        store.putJob(new TestJob(new JobStatus("jobId2", "nobody", JobStatus.Status.COMPLETED, 0, 0, 0, notsoold, now, null, null, null)))
        store.putJob(new TestJob(new JobStatus("jobId3", "nobody", JobStatus.Status.COMPLETED, 0, 0, 0, veryold, now, null, null, null)))
